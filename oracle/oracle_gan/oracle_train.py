...
def get_train_ops(config, g_pretrain_loss, g_loss, d_loss, global_step):
    ...
    # gradient clipping
    g_grads, _ = tf.clip_by_global_norm(tf.gradients(ys=g_loss, xs=g_vars), grad_clip)  # gradient clipping
    ...
    # gradient clipping
    d_grads, _ = tf.clip_by_global_norm(tf.gradients(ys=d_loss, xs=d_vars), grad_clip)  # gradient clipping
...
def get_metric_summary_op(config):
    ...
    metrics_sum = []
    if config['nll_oracle']:
        nll_oracle = tf.compat.v1.placeholder(tf.float32)
        metrics_pl.append(nll_oracle)
        metrics_sum.append(tf.compat.v1.summary.scalar('metrics/nll_oracle', nll_oracle))
